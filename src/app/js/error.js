/**
  Supplies ability to create custom error classes
...
/**
  ...
  @returns {Error}
    An error class that can be instantiated where needed.
...
error.create = function(data) {
  ...
  if ( hash[meta.name] ) {
    if ( bc[meta.name] === undefined ) {
      // Tests need to be able to delete and re-create errors
      // Should not be done otherwise
      delete hash[meta.name];
    } else {
